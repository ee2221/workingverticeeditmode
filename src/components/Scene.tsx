...
const VertexCoordinates = ({ position, onPositionChange }) => {
  ...
  return (
    <div className="absolute right-4 bottom-4 bg-[#1a1a1a] text-white/90 p-4 rounded-xl shadow-2xl shadow-black/20 border border-white/5 font-mono">
      <div className="space-y-2">
        {(['x', 'y', 'z'] as const).map((axis) => (
          <div key={axis} className="flex items-center gap-2">
            <label className="w-8 text-white/70 uppercase text-sm">{axis}:</label>
            <input
              type="number"
              value={position[axis].toFixed(3)}
              onChange={(e) => handleChange(axis, e.target.value)}
              className="bg-[#2a2a2a] border border-white/10 px-2 py-1 rounded w-24 text-right focus:outline-none focus:border-blue-500/50"
              step="0.1"
            />
          </div>
        ))}
...
const VertexCountSelector = () => {
  ...
  return (
    <div className="absolute left-1/2 top-4 -translate-x-1/2 bg-[#1a1a1a] text-white/90 p-4 rounded-xl shadow-2xl shadow-black/20 border border-white/5">
      <div className="flex items-center gap-2">
        <label className="text-sm font-medium text-white/70">Vertex Count:</label>
        <select
          className="bg-[#2a2a2a] border border-white/10 px-3 py-1.5 rounded text-sm focus:outline-none focus:border-blue-500/50"
